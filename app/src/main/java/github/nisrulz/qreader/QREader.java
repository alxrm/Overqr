...
/*
    ...
    if (barcodeDetector.isOperational()) {
      barcodeDetector.setProcessor(new Detector.Processor<Barcode>() {
        ...
        @Override public void receiveDetections(Detector.Detections<Barcode> detections) {
          final List<Barcode> barcodes = Lists.fromSparseArray(detections.getDetectedItems());

          if (barcodes.size() != 0 && qrDataListener != null) {
            operationHandler.post(new Runnable() {
              @Override public void run() {
                qrDataListener.onDetected(barcodes);
              }
            });
          }
        }
  ...
  public static final class Builder {
    ...
    public Builder operationHandler(@NonNull Handler handler) {
